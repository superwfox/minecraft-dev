...
**结构化 API 摘要**：每个已生成文件由 AI 提取结构化摘要（`FileSummary`），包含类名、公开方法签名、监听事件、注册命令、配置键等。这些摘要以可读的 API 签名块注入后续文件的生成 Prompt，精准传递跨文件依赖信息。

**为什么用结构化摘要而非完整代码**：
- Token 限制：一个 10 文件项目的全文可能超过 20K tokens
- 信息密度：后续文件只需要知道"前面文件暴露了什么可调用的接口"
- 防止虚空调用：明确约束"只能调用已列出的 API"，reChecker 同时验证跨文件调用一致性
