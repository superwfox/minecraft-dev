...
1. **澄清阶段**（多轮，按需）
   - AI 以 TodoList 形式追问关键决策（交互方式、数据持久化、增长曲线等）
   - 你逐题选择，AI 的思考过程实时展示

2. **规划阶段**（20-40 秒）
   - Planner 先确定主类蓝图（注册哪些命令 / 监听 / 任务）
   - 再产出带类型的文件树，按依赖深度划分并发桶

3. **生成阶段**（1-2 分钟）
   - 同一深度的文件并发生成（pom.xml、plugin.yml、Java 代码）
   - 每个文件经 reChecker 跨文件审查；发现缺失类自动补生，其他问题自动返工

4. **校验阶段**（5 秒）
   - 检查文件完整性，缺失则补齐

5. **构建阶段**（2-3 分钟）
   - 上传到 GitHub 临时分支，触发 Maven 构建
   - 编译失败时 AI 自动拉日志修复并重新构建

6. **完成**
   - 显示下载按钮，点击下载 JAR
   - 也可进入浏览器 IDE 在线编辑后再编译
...
- [浏览器 IDE](/features/ide)：生成后在线编辑代码
